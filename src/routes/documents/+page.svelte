<script lang="ts">
	...
	// My documents filtered from the subscription
	...
	<header class="border-b border-zinc-800/60 backdrop-blur-sm bg-zinc-950/80 sticky top-0 z-10">
		<div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
			<div class="flex items-center gap-3">
				<a href="/" class="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500/20 to-indigo-500/20 border border-violet-500/20 flex items-center justify-center">
					<span class="text-sm">📝</span>
				</a>
				<h1 class="text-lg font-semibold text-zinc-200 tracking-tight">Documents</h1>
			</div>
			...
			<div class="flex items-center gap-3">
				...
				<button onclick={() => { logout(); goto('/login'); }} class="btn-ghost text-sm">
					Sign out
				</button>
			</div>
		</div>
	...
	<main class="max-w-5xl mx-auto px-6 py-8">
		{#if ndk.$currentPubkey}
			<div class="mb-8 flex items-center gap-2 text-sm text-zinc-500">
				<User.Root {ndk} pubkey={ndk.$currentPubkey}>
					<User.Avatar class="w-6 h-6 text-[9px]" />
					<User.Name class="text-xs text-zinc-400" />
				</User.Root>
			</div>
		{/if}
		...
		{:else}
			<div class="text-center py-24 animate-fade-in">
				...
				<h2 class="text-lg font-medium text-zinc-300 mb-2">No documents yet</h2>
				<p class="text-zinc-500 text-sm mb-6">Create your first collaborative document to get started.</p>
				<a href="/documents/new" class="btn-primary inline-flex items-center gap-2">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
					</svg>
					Create document
